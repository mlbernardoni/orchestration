...
//		  cluster2.close();
	...
	protected String DoRoot ( JSONObject jsonrtoos) throws IOException {
      ...
      //session.execute("CREATE KEYSPACE IF NOT EXISTSS rtoos WITH replication = {'class': 'SimpleStrategy', 'replication_factor': '1'}  AND durable_writes = true;");
      //session.execute("CREATE TABLE IF NOT EXISTSS rtoos.service_tree (root_service UUID,parent_service UUID,service UUID,service_url text,service_param text,PRIMARY KEY(root_service, parent_service, service));");
      ...
      session2.execute("INSERT INTO service_tree (root_service, parent_service, service, service_url, service_param, servicetype, status) VALUES (?, ?, ?, ?, ?, ?, ?) IF NOT EXISTS;", 
    		  eventid, eventid, eventid, serviceurl, serviceparam, "R", "R");
	...
	protected String DoPre ( JSONObject jsonrtoos) throws IOException {
		  ...
		  stquery = "UPDATE service_tree SET servicetype  = 'S' WHERE root_service = ";
		  stquery += rootid;
		  stquery += " AND parent_service = ";
		  stquery += theparent;
		  stquery += " AND service = ";
	...
	protected String DoNew ( JSONObject jsonrtoos) throws IOException {
	      ...
	      //System.out.println( jsonrtoos.toString());
		  String rootid = jsonrtoos.getString("root_service");
	      ...
	      UUID eventid = UUID.fromString(jsonrtoos.getString("service"));
	      session2.execute("INSERT INTO service_tree (root_service, parent_service, service, service_url, service_param, status, servicetype) VALUES (?, ?, ?, ?, ?, ?, ?) IF NOT EXISTS;", 
	    		  UUID.fromString(rootid), UUID.fromString(partentid), eventid, serviceurl, serviceparam, "R", servicetype);
...
//System.out.println(testservice);
		      ...
		      String stquery = "SELECT status, servicetype, service  FROM service_tree WHERE ";
		      stquery += "root_service = ";
		      stquery += rootid;	
		      stquery += " AND parent_service = ";
		      stquery += testservice;
...
//	   	  Cluster cluster2 = Cluster.builder().addContactPoint("127.0.0.1").build();
	...
	//
	// /////////////////////////////////////////////
	protected String DoBatch( JSONObject jsonrtoos) throws IOException {
		String sttype = null;
		String strep = null;
		  try 
		  {
